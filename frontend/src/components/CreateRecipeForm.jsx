...
import { LuDivide, LuLeaf } from "react-icons/lu";
...
export default function CreateRecipeForm() {
  const handleAddIngredient = () => {
    // UPDATE VALIDATION
    // not same ingredient name
    if (!ingrAmount || !ingrName.trim() || !ingrUnit.trim()) return;
    const newIngredient = {
      name: ingrName,
      amount: ingrAmount,
      unit: ingrUnit,
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-7">
      ...
      <div className="relative" ref={dietDropdownRef}>
        <div className={`${labelClasses} flex justify-between w-full`}>
          <span className="self-start">Diet</span>
          {diet.length > 0 && (
            <span className="text-sm self-end italic">
        ...
        <div className="flex flex-wrap gap-2 px-3 pt-0.5 pb-1">
